...
shared_examples_for 'config with pki = false and firewall = false' do
  ...
  it { is_expected.not_to contain_pki__copy('freeradius') }
  it { is_expected.not_to create_file('/etc/pki/simp_apps/freeradius/x509') }
  it { is_expected.not_to contain_iptables__listen__udp('radius_iptables_udp') }
  it { is_expected.not_to contain_iptables__listen__tcp_stateful('radius_iptables_tcp') }
end
...
shared_examples_for 'config v3' do
  ...
  it_behaves_like 'v3 conf users default'
...
shared_examples_for 'v3 conf users default' do
  ...
  it { is_expected.to contain_concat__fragment('radius_user_header').with_content(%r{# This file is managed by Puppet.}) }
end
      ...
      context "on #{os}" do
        ...
        context 'version 3 (v3)' do
          ...
          context 'with default parameters' do
            ...
            it_behaves_like 'config with pki = false and firewall = false'
          ...
          context 'with pki = true' do
            ...
            it_behaves_like 'config with pki = true'
          end
          ...
          context 'with pki = simp and firewall = true' do
            ...
            it_behaves_like 'install'
            it_behaves_like 'config with pki = simp and firewall = true'
            it_behaves_like 'config v3'
            it_behaves_like 'service'
          end
          ...
          context 'with use_rsync_radiusd_conf = true' do
            ...
            it_behaves_like 'use_rsync and testcerts'
          end
          ...
          context 'v3::conf with changed parameters' do
            ...
            it { is_expected.to contain_iptables__listen__udp('radius_iptables_udp') }
            it { is_expected.to contain_iptables__listen__tcp_stateful('radius_iptables_tcp') }
            it { is_expected.not_to contain_class('freeradius::v3::conf::users') }
        ...
        context 'version 2 (v2)' do
          ...
          context 'default params' do
            it 'fails' do
              expect do
                is_expected.to raise_error(Puppet::Error, %r{This module is designed to work with freeradius version 3.X})
              end
            end
          end
          context 'v2 with rsync ' do
            ...
            it_behaves_like 'use_rsync and testcerts'
            it { is_expected.not_to contain_class('freeradius::v3::conf') }
            it_behaves_like 'config with pki = false and firewall = false'
          end
        end
      end
